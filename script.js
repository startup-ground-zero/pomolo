...
if (catalogueViewer) {
  ...
  document.querySelectorAll('.catalogue-card').forEach(card => {
    const openCatalogueViewer = () => {
      ...
      catalogueTitle.textContent = `${catalogueName} Catalogues`;
      cataloguePreviewTitle.textContent = `${catalogueName} Catalogues`;
      ...
      for (let index = 1; index <= 5; index += 1) {
        ...
        itemTitle.textContent = `${catalogueName} Catalogue ${index}`;
